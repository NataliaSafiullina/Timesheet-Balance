Репозиторий Timesheet-Balance-Skillbox содержит проект,
созданный по техническому заданию:

Ваша задача состоит в том, чтобы реализовать для программы следующую
функциональность, доступную через командную строку:
...
В примерах выше program — имя вашей программы. Оно зависит от того, на каком языке
программирования вы будете выполнять данный проект. Пример для Java:
...
– DDL для создания триггеров, console_mysql.sql 
– концептуальная модель БД, Balance_DB_Model.png
